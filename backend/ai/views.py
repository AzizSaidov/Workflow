...
async def _request(messages: list[dict], max_tokens: int, temperature: float) -> str:
    if not GROQ_KEY:
...
async def _call(messages: list[dict], max_tokens: int = 1200,
                temperature: float = 0.7, forbid_foreign: bool = False) -> str:
    content = await _request(messages, max_tokens, temperature)
    if forbid_foreign and _has_foreign(content):
        retry_messages = messages + [
            {"role": "assistant", "content": content},
            {"role": "user", "content": (
                "В твоём ответе оказались символы другого алфавита (иероглифы или арабская вязь). "
                "Перепиши ТОТ ЖЕ текст без них, сохранив исходный язык и смысл "
                "(технические термины и названия можно латиницей). Верни только готовый текст."
            )},
        ]
        retry = await _request(retry_messages, max_tokens, 0.2)
        content = retry if not _has_foreign(retry) else _strip_foreign(retry)
    return content
...
ПРАВИЛА:
- Пиши на русском языке. Без иероглифов и арабских символов — только кириллица и латиница (для названий/терминов)
...
async def help_project(title: str, rough_description: str, category: str, budget: str) -> str:
    ...
    ], max_tokens=800, forbid_foreign=True)
...
ПРАВИЛА:
- Пиши СТРОГО на русском языке от первого лица. Технические термины и названия — латиницей (React, Node.js, PostgreSQL). НИКОГДА не вставляй иероглифы, китайские/японские/корейские или арабские символы
...
async def help_bid(project_title: str, project_description: str, skills: list[str],
    ...
    return await _call([
        ...
        {"role": "user", "content": user_msg},
    ], max_tokens=600, forbid_foreign=True)
...
ПРАВИЛА:
- Пиши на русском языке, от первого лица. Без иероглифов и арабской вязи — только кириллица и латиница (для названий/терминов)
...
async def help_deliver(project_title: str, project_description: str,
    ...
    return await _call([
        ...
        {"role": "user", "content": user_msg},
    ], max_tokens=450, forbid_foreign=True)
...
async def ai_chat(message: str, history: list, context: str | None) -> str:
    ...
    return await _call(messages, max_tokens=1000, forbid_foreign=True)
