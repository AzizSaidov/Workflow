...
from contracts.views import get_my_contracts, get_contract, complete_contract, get_contract_by_project
...
@contracts_router.get("/project/{project_id}", response_model=ContractResponse | None)
def contract_by_project(project_id: UUID, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return get_contract_by_project(project_id, current_user, db)
