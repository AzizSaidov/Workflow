# Workflow — ТЗ
### Версия 6.0 | Обновлено 2026-06-02
...
```
ПРИОРИТЕТ 1 — Редизайн страниц:
  1. Profile (sidebar слева + вкладки + Certifications CRUD)
  2. ProjectDetail (вкладки + Dispute кнопка + Contracts секция)
  3. AIAssistant (sidebar + split layout)
  4. Wallet (большой баланс + фильтр транзакций)

ПРИОРИТЕТ 2 — Пропущенные фичи:
  5. Client Profile страница (/client/:id или расширить /profile)
  6. Report кнопка (FreelancerCard + ProjectDetail)

ПРИОРИТЕТ 3 — UX улучшения:
  7. Freelancers (убрать ProtectedRoute + поиск + фильтры)
  8. Пагинация в ProjectsFeed
  9. URL-синхронизация фильтров

ПРИОРИТЕТ 4 — Техническое:
  10. useSEO хук + применить на 5 страницах
  11. Alembic check (проверить что миграции в порядке)
  12. Async бэкенд (если есть время)

ФИНАЛ:
  13. git pull + npm run build на сервере
  14. Проверить все сценарии: гость / фрилансер / клиент / admin
  15. OpenDay презентация ✅
```
...
Переписать:
  frontend/src/pages/Profile.jsx            (sidebar слева + вкладки)
  frontend/src/pages/ProjectDetail.jsx      (вкладки + спор + контракт)
  frontend/src/pages/AIAssistant.jsx        (sidebar + split)
...
*Workflow TZ v6.0 | 2026-06-02 | AzizSaidov*
