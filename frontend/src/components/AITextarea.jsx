    ...
    try {
      ...
      if (action === 'generate' && aiContext?.mode === 'bid') {
        ...
        const { data } = await aiApi.chat(
          `Улучши этот текст, сделай его более профессиональным и убедительным. Верни только улучшенный текст без комментариев:\n\n${value}`
        )
        result = data.text

      } else if (action === 'shorten') {
        const { data } = await aiApi.chat(
          `Сократи этот текст, оставив главное. Верни только сокращённый текст без комментариев:\n\n${value}`
        )
        result = data.text
