...
### Профили
- Логика: `profiles/views.py`. Навыки/языки/специализация — **только из справочников**
  (`/api/skills`, `/api/languages`, `/api/categories`), не свободный ввод.
- **Специализация** фрилансера — мультивыбор категорий (M2M `ProfileCategory`). Раньше рядом
  было дублирующее одиночное поле `category_id` — **убрано**; фильтр на `/freelancers` идёт по M2M.
- Профиль заказчика (`/client-profiles/{id}`): если строки профиля ещё нет (напр. у админа),
  отдаётся пустой профиль вместо 404 — страница рендерится целиком, а не «наполовину».
...
- На фронте только что заработанная ачивка «въезжает» тостом справа (как в Steam):
  `AchievementToast` рендерится через **портал в `body`** (иначе `backdrop-filter` навбара
  ломал позиционирование); дедуп по `localStorage` + окно свежести (`NotificationBell`).
...
| `help_bid` | проект + **реальный профиль** фрилансера (навыки/опыт/рейтинг) + его черновик | cover letter, **без выдуманных фактов** |
| `help_deliver` | **заметки фрилансера** о сделанном + ссылки (контекст проекта) | оформляет отчёт о сдаче (без заметок — 422, не выдумывает) |
| `edit_text` | текст из textarea | **перевод** на English (улучшить/сократить убраны из UI — давали слабый результат) |
...
- `AITextarea` (`components/AITextarea.jsx`) — переиспользуемый textarea с ✨-меню
  (**генерация + перевод**), работает в режимах `bid` / `project` / `deliver`.
...
- Защита от «языковых протечек» llama: если в ответ просочились иероглифы/арабица —
  делается корректирующий ретрай на низкой температуре, иначе символы вычищаются
  (`_call(forbid_foreign=True)` в `ai/views.py`).
